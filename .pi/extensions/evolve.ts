...
function doHelp(ctx: ExtensionContext): void {
    const lines = [
        ...
        "      • Chat (Telegram/Slack/A2A): `reload_extensions` tool (admin-only)",
        "      • TUI:                       `/reload` slash command",
        "  This slash command gives you the surface to discover state",
        "  (list, diff) and tells the agent the workflow. The actual",
        "  write + reload mechanics are Pi-native + ChannelRuntime-backed.",
        "",
        ...
        "    6. reload_extensions()   (chat)   OR   /reload   (TUI)",
        "    7. test the new tool on the NEXT message",
